...
void Asteroids::Start()
{
	...
	Animation *asteroid_anim = AnimationManager::GetInstance().CreateAnimationFromFile("asteroid1", 128, 8192, 128, 128, "asteroid1_fs.png");
...
void Asteroids::OnKeyPressed(uchar key, int x, int y)
{
	if (!mGameStarted)
	{
		...
		case ' ':
			...
			mGameWorld->FlagForRemoval(mAlien);
		}
	}
	else
	{
...
void Asteroids::OnObjectRemoved(GameWorld* world, shared_ptr<GameObject> object)
{
	if (object->GetType() == GameObjectType("Alien"))
	{
		if (!mGameStarted)
		{
			...
			explosion->SetPosition(mAlien->GetPosition());
			explosion->SetRotation(mAlien->GetRotation());
	...
	if (object->GetType() == GameObjectType("Power"))
	{
		if (!mGameStarted)
		{
			if (mAlien->mShield == true)
			{
				if (mAlien->mShieldlevel2 == true)
				{
					mAlien->mShieldlevel3 = true;
				}
				mAlien->mShieldlevel2 = true;
			}
			mAlien->mShield = true;
		}
		else
		{
			if (mSpaceship->mShield == true)
			{
...
void Asteroids::OnTimer(int value)
{
	...
	if (value == ENEMY_SHOOT)
	{
		...
		//mAlien->Thrust(rand() % 11 + (-2));
		////mAlien->Rotate(rand() % 120  + (-60));
		//mAlien->Rotate(mSpaceship->GetPosition().y - mSpaceship->GetPosition().x);
		//mAlien->Shoot();
		//SetTimer(1000, ENEMY_SHOOT);
		if (!mGameStarted)
		 {
			mAlien->Thrust(rand() % 12 + (-2));
			mAlien->Rotate(rand() % 120 + (-60));
			mAlien->Shoot();
			SetTimer(1000, ENEMY_SHOOT);
			}
		if (mGameStarted)
			{
			mAlien->Thrust(rand() % 10 + (-2));
			mAlien->Rotate(mSpaceship->GetPosition().x - mSpaceship->GetPosition().y);
			mAlien->Shoot();
			SetTimer(1000, ENEMY_SHOOT);
			}
	}
...
shared_ptr<GameObject> Asteroids::CreateAlien()
{
	...
	mAlien->SetBoundingShape(make_shared<BoundingSphere>(mAlien->GetThisPtr(), 4.0f));
	shared_ptr<Shape> enemy_bullet_shape = make_shared<Shape>("bullet.shape");
	mAlien->SetAlienBulletShape(enemy_bullet_shape);
	Animation *anim_ptr = AnimationManager::GetInstance().GetAnimationByName("spaceship");
	shared_ptr<Sprite> spaceship_sprite =
		make_shared<Sprite>(anim_ptr->GetWidth(), anim_ptr->GetHeight(), anim_ptr);
	...
	// Reset spaceship back to centre of the world
	mAlien->Reset();
	mAlien->SetPosition(GLVector3f(-20, 20, 0));
	// Return the spaceship so it can be added to the world
	return mAlien;

}

void Asteroids::CreatePower(const uint num_power)
{
	mPowerCount = num_power;

	for (uint i = 0; i < num_power; i++)
	{
		shared_ptr<GameObject> power = make_shared<Power>();
		power->SetBoundingShape(make_shared<BoundingSphere>(power->GetThisPtr(), 5.0f));
		power->SetScale(0.2f);
		mGameWorld->AddObject(power);
	}
}
...
void Asteroids::CreateGUI()
{
	...
	// Create a new GUILabel for the start label and wrap it up in a shared_ptr
	mStartLabel = make_shared<GUILabel>("Press any key to begin");
